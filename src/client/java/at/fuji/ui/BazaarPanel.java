...
public class BazaarPanel implements Sidebar {
    ...
    // Row 1: [NPC SELL 68] [DEBUG 56] gap [Sells/hr lbl] [-] [value] [+] gap [Vol/wk lbl] [-] [value] [+]
    ...
    // Row 2: [● 14] [Profit/hr lbl] [-] [value] [+]  gap  [● 14] [Total profit lbl] [-] [value] [+]
    ...
    // ── Enums ─────────────────────────────────────────────────────────────────
    private enum Tab { TOP, REJECTED, BLACKLIST }
    ...
    @Override public String getLabel()    { return "BAZAAR"; }
    @Override public int getAccentColor() { return COL_ACCENT; }

    @Override
    public void init(FujiScreen parent, int px, int py, int pw, int ph) {
        this.parent = parent; this.px = px; this.py = py; this.pw = pw; this.ph = ph;
        scroll = 0; selectedIdx = -1; selectedItem = null;
        rebuildSortedList();
        rebuildWidgets();
    }
    ...
    public boolean mouseScrolled(double mx, double my, double amount) {
        if (mx < px || mx > px + pw || my < listTop() || my > listBot()) return false;
    ...
    public boolean onMouseClicked(double mx, double my) {
        if (tab == Tab.TOP) {
            ...
            if (my >= colHdrY && my < colHdrY + COL_HDR_H) {
                if      (mx >= profitCol() && mx < totalCol())  { setSortMode(SortMode.PROFIT_HR); return true; }
                else if (mx >= totalCol()  && mx < fillCol())   { setSortMode(SortMode.TOTAL);     return true; }
                else if (mx >= fillCol()   && mx < spreadCol()) { setSortMode(SortMode.FILL);      return true; }
                else if (mx >= spreadCol() && mx < btnCol())    { setSortMode(SortMode.SPREAD);    return true; }
            }
            // Row selection
    ...
    private int btnCol()    { return px + pw - PADDING - SB_W - 20; }
    private int spreadCol() { return btnCol()    - 56; }
    private int fillCol()   { return spreadCol() - 58; }
    private int totalCol()  { return fillCol()   - 68; }
    private int profitCol() { return totalCol()  - 72; }
    private int nameCol()   { return px + PADDING + 22; }

    // ── Layout helpers ────────────────────────────────────────────────────────
    private int listTop() { return py + HEADER_H + SETTINGS_H + TAB_H; }
    private int listBot() { return (tab == Tab.BLACKLIST) ? py + ph - FOOTER_H : py + ph; }
    private int listCount() {
        return switch (tab) {
            case TOP      -> sortedList.size();
            case REJECTED -> { var r = ItemSelector.lastResult; yield r != null ? r.rejected.size() : 0; }
    ...
    private void setSortMode(SortMode m) {
        sort = m; scroll = 0; selectedIdx = -1; selectedItem = null;
        rebuildSortedList(); rebuildWidgets();
    }

    private void rebuildSortedList() {
        ItemSelector.ScoringResult result = ItemSelector.lastResult;
        if (result == null) { sortedList = new ArrayList<>(); return; }
    ...
    private void addBtn(ButtonWidget b)    { managedWidgets.add(b); parent.addWidget(b); }
    ...
    private void rebuildWidgets() {
        ...
        addBtn(btn(loading ? "..." : "\u21BB REFRESH",
            ...
            if (loading) return;
            loading = true; selectedIdx = -1; selectedItem = null; rebuildWidgets();
            ItemSelector.scoreAllItems(BazaarWorker.lastKnownPurse).thenAccept(result -> {
                loading = false;
                MinecraftClient.getInstance().execute(() -> { rebuildSortedList(); rebuildWidgets(); });
            });
        }));
        ...
        addBtn(tog("NPC SELL", cfg.npcSellMode, px + R1_NPC_X, r1Y, TOG_W, b -> {
            cfg.npcSellMode = !cfg.npcSellMode; ModConfig.save(); rebuildWidgets(); }));
        addBtn(tog("DEBUG", cfg.debugMode, px + R1_DBG_X, r1Y, 56, b -> {
            cfg.debugMode = !cfg.debugMode; ModConfig.save(); rebuildWidgets(); }));

        // Sells/hr [-] [+]
        addBtn(sm("-", px + R1_SMN_X, r1Y, () -> {
            cfg.minSellsPerHour = Math.max(0, cfg.minSellsPerHour - 10); ModConfig.save(); rebuildWidgets(); }));
        addBtn(sm("+", px + R1_SPL_X, r1Y, () -> {
            cfg.minSellsPerHour += 10; ModConfig.save(); rebuildWidgets(); }));

        // Vol/wk [-] [+]
        addBtn(sm("-", px + R1_VMN_X, r1Y, () -> {
            cfg.minWeeklyVolume = Math.max(0, cfg.minWeeklyVolume - 100_000); ModConfig.save(); rebuildWidgets(); }));
        addBtn(sm("+", px + R1_VPL_X, r1Y, () -> {
            cfg.minWeeklyVolume += 100_000; ModConfig.save(); rebuildWidgets(); }));
        ...
        addBtn(enTog(cfg.minProfitPerHourEnabled, px + R2_PEN_X, r2Y, b -> {
            cfg.minProfitPerHourEnabled = !cfg.minProfitPerHourEnabled; ModConfig.save(); rebuildWidgets(); }));
        addBtn(sm("-", px + R2_PMN_X, r2Y, () -> {
            cfg.minProfitPerHour = Math.max(0, cfg.minProfitPerHour - 5_000); ModConfig.save(); rebuildWidgets(); }));
        addBtn(sm("+", px + R2_PPL_X, r2Y, () -> {
            cfg.minProfitPerHour += 5_000; ModConfig.save(); rebuildWidgets(); }));

        // Total profit
        addBtn(enTog(cfg.minTotalProfitEnabled, px + R2_TEN_X, r2Y, b -> {
            cfg.minTotalProfitEnabled = !cfg.minTotalProfitEnabled; ModConfig.save(); rebuildWidgets(); }));
        addBtn(sm("-", px + R2_TMN_X, r2Y, () -> {
            cfg.minTotalProfit = Math.max(0, cfg.minTotalProfit - 10_000); ModConfig.save(); rebuildWidgets(); }));
        addBtn(sm("+", px + R2_TPL_X, r2Y, () -> {
            cfg.minTotalProfit += 10_000; ModConfig.save(); rebuildWidgets(); }));
        ...
        if (tab == Tab.TOP) {
            for (int i = 0; i < sortedList.size(); i++) {
                ...
                if (rowY + ROW_H <= rowsTop || rowY >= listBot) continue;
                addBtn(btn("\u25BA", btnCol() + 1, rowY + (ROW_H - 12) / 2, 18, 12, b -> {
                    if (BazaarWorker.isEnabled()) BazaarWorker.stop();
            ...
            for (int i = 0; i < bl.size(); i++) {
                ...
                addBtn(btn("REMOVE", px + pw - PADDING - SB_W - 46,
                    ...
                    cfg.bazaarBlacklist.remove(idx); ModConfig.save(); rebuildWidgets(); }));
            }
            // Footer
            ...
            if (addRowVisible) {
                ...
                addBtn(btn("ADD", px + PADDING + 208, footerY + 11, 36, 14, b -> {
                    ...
                    if (!id.isEmpty() && !cfg.bazaarBlacklist.contains(id)) {
                        cfg.bazaarBlacklist.add(id); ModConfig.save(); }
                    addRowVisible = false; rebuildWidgets(); }));
                addBtn(btn("CANCEL", px + PADDING + 250, footerY + 11, 48, 14, b -> {
                    addRowVisible = false; rebuildWidgets(); }));
            } else {
                addBtn(btn("+ BLACKLIST ITEM", px + PADDING, footerY + 11, 110, 14, b -> {
                    addRowVisible = true; rebuildWidgets(); }));
            }
        }
    ...
    private void switchTab(Tab t) {
        tab = t; scroll = 0;
        if (t != Tab.TOP) { selectedIdx = -1; selectedItem = null; }
        rebuildWidgets();
    }
    ...
    public void render(DrawContext gfx, int mouseX, int mouseY, float delta) {
        ...
        if (selectedItem != null) renderDetail(gfx, tr);
    }
    ...
    private void renderDetail(DrawContext gfx, TextRenderer tr) {
        ...
        gfx.drawText(tr, trunc(tr, s.displayName, pvW - DET_PAD * 2 - 16), pvX + DET_PAD, cy, COL_TEXT, false); cy += 11;
        gfx.drawText(tr, s.productId, pvX + DET_PAD, cy, COL_TEXT_DIM, false); cy += 13;
        div(gfx, pvX, pvW, cy); cy += 6;

        drow(gfx, tr, pvX, pvW, cy, "Ask price",  fmtD(s.askPrice),  COL_RED);   cy += DET_ROW;
        drow(gfx, tr, pvX, pvW, cy, "Bid price",  fmtD(s.bidPrice),  COL_GREEN); cy += DET_ROW;
        int npcCol = s.npcSellPrice > s.askPrice ? COL_GREEN : COL_TEXT_DIM;
        drow(gfx, tr, pvX, pvW, cy, "NPC sell", s.npcSellPrice > 0 ? fmtD(s.npcSellPrice) : "N/A", npcCol); cy += DET_ROW;
        div(gfx, pvX, pvW, cy); cy += 6;

        drow(gfx, tr, pvX, pvW, cy, "Spread",
                fmtD(s.spread) + " (" + String.format("%.1f", s.spreadPercent * 100) + "%)",
                COL_YELLOW); cy += DET_ROW;
        div(gfx, pvX, pvW, cy); cy += 6;

        drow(gfx, tr, pvX, pvW, cy, "Fill/hr",     String.format("%.0f", s.fillRatePerHour), COL_TEXT); cy += DET_ROW;
        drow(gfx, tr, pvX, pvW, cy, "Buy vol/wk",  fmtD(s.buyMovingWeek),  COL_TEXT); cy += DET_ROW;
        drow(gfx, tr, pvX, pvW, cy, "Sell vol/wk", fmtD(s.sellMovingWeek), COL_TEXT); cy += DET_ROW;
        div(gfx, pvX, pvW, cy); cy += 6;

        drow(gfx, tr, pvX, pvW, cy, "BZ profit/hr",   fmtD(s.weeklyProfit), COL_GREEN); cy += DET_ROW;
        drow(gfx, tr, pvX, pvW, cy, "BZ total profit", fmtD(s.totalProfit),  COL_GREEN); cy += DET_ROW;
        drow(gfx, tr, pvX, pvW, cy, "Order amount",    String.valueOf(s.purchasableAmount), COL_TEXT); cy += DET_ROW;

        if (s.npcSellPrice > s.askPrice) {
            div(gfx, pvX, pvW, cy); cy += 6;
            drow(gfx, tr, pvX, pvW, cy, "NPC profit/hr", fmtD(s.npcProfitPerHour), COL_CYAN); cy += DET_ROW;
            drow(gfx, tr, pvX, pvW, cy, "NPC per item",  fmtD(s.npcProfitPerItem), COL_CYAN); cy += DET_ROW;
        }

        if (s.manipulated) {
            cy += 4; div(gfx, pvX, pvW, cy); cy += 6;
            gfx.fill(pvX + DET_PAD, cy - 2, pvX + pvW - DET_PAD, cy + 10, 0x44FF4455);
            gfx.drawText(tr, "\u26A0 LIKELY MANIPULATED", pvX + DET_PAD, cy, COL_RED, false); cy += DET_ROW;
        }
